...
const { normBrand, slugMatchScore } = require('./normalize');
...
function indexUrls(db, shopCfg, catalog, rawUrls) {
  ...
  for (const url of productUrls) {
    ...
    // Zoek best-matchende model in de catalogus
    let bestModel = null, bestScore = 0;
    for (const [modelKey] of catalog.models) {
    ...
    upsertIndex(db, { shop: key, normBrand: nb, normModel: bestModel, title: slug, url, platform: 'custom' });
