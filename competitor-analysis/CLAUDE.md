...
- **Real prices** (each cracked by reverse-engineering its configurator):
  ...
  - `13-horrenmax` — Shopify + **DPO (Itoris)**, **cm input** (max 210×300), price in `.s1pr.f8pr-price .regular`. Works, BUT the DPO bundle loads from node1.itoris.com behind **bot protection** that rate-limits headless runs — so it often yields `n.v.t.` (spec reloads a few times to catch it when it does load).
