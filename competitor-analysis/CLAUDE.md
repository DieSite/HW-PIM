...
npm run test:eigen            # single competitor (see package.json for the full set:
                              #   eigen, horrentotaal, horrengigant, horren-com, creon,
                              #   koopje-horren, horrenstunter, solano)
npm run test:kolommen         # browserloos: kolomvolgorde + koppeling spec <-> Excelkolom
...
| Spec | Site | Status |
|---|---|---|
| 01 | plissehordeurenwebshop.nl (**own shop**) | ✅ real per-size prices. Progressive JS configurator: `a.js-config-button` → `label[for=op_de_dag-1]` → `#breedte`/`#hoogte` → `schuin-1`/framekleur/gaas/handgreep/powertape labels. Price = "Hordeur" row current (non-struck) price in `.configurator__totals` **minus the "Korting" promo row** (bv. "Zomerkorting 2026"), and still **excluding** the afhaalkorting that sits in the Totaal. Rekenregel staat in `tests/_eigenwinkel.js` (unit tests: `npm run test:eigenwinkel`) — bewust buiten de browser zodat hij te testen is. **Valkuil:** de Levering-regel héét zelf "Afhalen in Gorinchem (extra korting)", dus matchen op de tekst "korting" trekt die €5,20 er ten onrechte ook af; match op de rijkop. Vóór 2026-07-29 werd de promo helemaal genegeerd en stond onze eigen winkel dus structureel te duur in de vergelijking (730×1970 zwart: € 289 i.p.v. € 260). |
| 03 | horrentotaal.nl | ✅ **real per-size, no browser page** (herschreven 2026-07-30). Hun Shopify-front gaf headless Chromium `429 local_rate_limited` waardoor de spec 0 van de 34 cellen vulde; `configurator.horrentotaal.nl` is wél gewoon met de `request`-fixture te bevragen. Prijs = `POST /calculate/<slug>` (`totaalPrijs`; slugs `plisse` en `dubbele_plisse_hordeur` uit `GET /configurators`), plus de grijs-gaastoeslag uit `/configurators/<slug>`, **minus the running shop promotion** from `/active-discounts?productId=<id>`. **Valkuil:** `calculate` weigert een te grote maat niet maar knijpt hem terug naar de grootste band (3000 mm → prijs van 2000 mm) — daarom toetsen we aan de `afmeting`-grenzen uit hun config én eisen we `matchedBreedte/matchedHoogte >= onze maat`. De winkel wordt nog hooguit 2× per run geraakt (product-id per deurtype uit `/products/<handle>.js`, gecachet). The discount is in neither the calculate response nor the config — their widget subtracts it client-side and it is what lands in the cart, so leaving it out priced horrentotaal 10% too high (fixed 2026-07-29). Rekenregel in `tests/_korting.js` (unit tests: `npm run test:korting`), bewust buiten de browser — zelfde opzet als `_eigenwinkel.js`. Product-id verschilt per pagina → uit de DOM lezen, nooit hardcoden. **Valkuil:** de korting geldt óók over de meerprijs voor grijs gaas, en een `volume`-korting (vanaf x stuks) juist níét. Lukt het ophalen van de kortingen niet, dan `n.v.t.` — anders leggen we stilletjes weer een te hoge prijs vast. |
...
configurator:
  ...
  - `20-solano-wonen` ✅ per-maat, **no browser page** — eigen shopplatform (niet Shopify/WooCommerce). Configurator praat met `POST /services/getProductConfiguration` (form-encoded `csrfToken` + `productId` + genummerde `options[ID]`), gevonden door de live pagina te draaien met `page.on('response')` terwijl breedte/hoogte werden ingevuld. **Product gewisseld 2026-07-30 (klantverzoek): Keje plissehordeur (`productId=991`) i.p.v. de Luxaflex Volare (581).** De Volare is een dealerproduct in een andere prijsklasse (€448 vs €267 voor 730×1970) én de enige van hun plissé hordeuren zónder de optie **"Op maat zagen"** — zonder die optie krijg je een zelf in te korten bouwpakket, niet vergelijkbaar met de kant-en-klare deuren van de andere bronnen (zelfde afweging als bij Handige Horren). Optie-ID's: deurtype `options[3916]` (`31363`=enkel, `31366`=dubbel), breedte/hoogte (**cm**) `options[3919]`/`options[3922]`, profielkleur `options[3925]=31372` + `options[3928]=31387` (Zuiverwit RAL 9010), montage in het kozijn `options[3937]=31399` (enkel) / `options[4840]=40241` (dubbel), **op maat zagen `options[4842]=40246` (+€24,95)**. Hun 20% winkelkorting zit al in `total` (advies €302,50 → €242,00). Dit product heeft géén gaaskleur-optie → grijs = n.v.t. **Val niet terug op eigen min/max-aannames**: de API keurt combinaties af via een niet-triviale breedte×hoogte-matrix per deurtype en geeft bij een te grote maat gewoon een (verkeerd) totaalbedrag terug — vertrouw op het eigen `errors`-object (alle velden leeg = geldig) plus `priceGroupDetails.inRange`. **Cookie/CSRF-val**: de bootstrap-GET (voor het csrfToken) moet **per test** opnieuw, niet gecached over tests heen — Playwright geeft elke `test()` een eigen lege `request`-context, dus een gecachet token zonder de bijbehorende sessiecookie van dezelfde GET geeft vanaf de tweede test een foutieve/sessieloze POST (in dit geval stilletjes terugvallend op een generieke fallback-prijs, geen crash — zo'n stille bug valt alleen op door de resultaten te controleren, niet aan een groene testrun). `fittingAmount` (optionele opmeetservice aan huis) telt niet mee in de genoteerde prijs. **De 20% korting geldt óók over "op maat zagen"** — geverifieerd 04-08-2026 met twee identieke calls die alleen in `options[4842]` verschilden: Nee = total €242,00 / advies €302,50, Ja = total €266,95 / advies €333,69. Beide totalen zijn dus al gekort (302,50 × 0,8 en 333,69 × 0,8) en de €24,95 die de pagina bij de optie toont is de gekorte meerprijs, niet de adviesprijs. `total` overnemen zoals hij is; er zelf nog eens 20% over de optie afhalen (→ €261,96) past de korting dubbel toe.
  - `18-horrenstunter` ✅ per-maat, enkel én dubbel — WooCommerce + **Gravity Forms** (form 134, identiek op beide productpagina's). Velden zijn conditioneel/readonly, dus JS-set + `gformCalculateTotalPrice()` en dan pollen op `.formattedTotalPrice`. Radio `input_93` is de deurkeuze (géén montagekeuze) en bepaalt wélk breedteveld telt: `input_97` (300–1900 mm) voor enkel, **`input_109` (300–3800 mm) voor dubbel**; hoogte is voor beide `input_70` (1791–2750). **Gecorrigeerd 2026-07-30:** de spec ging ervan uit dat de dubbele variant geen invulbare maatvelden toont en gaf dubbele openingen daarom de prijs van één enkele deur (t/m 1900 mm, breder n.v.t.) — `input_109` bestaat wel degelijk. Gaas alleen zwart → grijs = n.v.t.
  ...
  - `16-koopje-horren` ✅ per-maat, **no browser page** (herschreven 2026-07-30) — Bruynzeel Plissé Hordeur s900 op maat (enkel + dubbel). Magento met de plugin **`MageArray_Customprice`**, die de complete prijstabel als JSON in de productpagina zet (`optionConfig.pricesheet`, rijen = hoogte-, kolommen = breedtebanden, `"x"` = maken ze niet). Eén `request.get()` per deurtype volstaat; rekenregel + gaasoptie in `tests/_koopje.js` (unit tests `npm run test:koopje`). **Valkuil die hier jarenlang in zat:** de getoonde €179 / €349 is de goedkoopste cel (580×1791 resp. 1160×1791), niet een vaste prijs — de oude spec noteerde die voor alle 34 maten en zette koopje-horren stelselmatig te goedkoop neer (730×1970 is in werkelijkheid €233, 1430×1970 €457). Grijs gaas kost niets extra maar kan maar tot 2600 mm hoog; die grens lezen we uit de optietitel ("Grijs (maximaal 2600mm hoog)") in plaats van hem te hardcoden.
...
**Recurring gotchas across configurators** (check these first when a price won't move): (1) dimensions wanted in **cm not mm** (horrengigant, horren.com); (2) recalc fires on **keyup**, so `.fill()` does nothing — use `pressSequentially` or JS-set + dispatch input/change/keyup (creon, horrenconcurrent, luxehorren); (3) the price is in a **hidden API response** — capture it with `page.on('response')` (creon `/product/price`) of roep de API direct aan via de `request`-fixture als dat zonder paginasessie werkt (horren.com `validate-state`, horrentotaal `configurator.horrentotaal.nl/calculate`, solano `getProductConfiguration`); soms staat de hele prijstabel zelfs gewoon in de pagina-HTML (koopje-horren `optionConfig.pricesheet`, decozijn); (4) a **cookie overlay** silently blocks interaction until dismissed — let op: de knoptekst "Accepteren" zit pas sinds 2026-06 in `acceptCookies`, en `playwright.config.js` heeft nu `actionTimeout: 10_000` zodat een afgedekt element een test nooit meer tot de test-timeout laat hangen.
...
Do **not** fabricate prices for the label/n.v.t. sites — the file is a real business comparison. The Excel Info sheet spells out the method per source, plus a **WIJZIGINGEN**-blok met de doorgevoerde bronconcorrecties per datum en hun effect op 730×1970 enkel — dáár hoort het antwoord op "wat is er eigenlijk aangepast?" te staan, niet in de git-log. When adding a competitor, also add its `{key,label}` to `COMPETITORS` in `globalTeardown.js` (the key must match the `recordPrice` competitor string).

**`kolommen.test.js` (`npm run test:kolommen`) bewaakt die redundantie.** Browserloos, milliseconden, en het faalt vóór een run van een kwartier. Het pint de door de klant vastgestelde kolomvolgorde vast (letterlijke lijst in de test — een gewenste wijziging pas je bewust op twee plekken aan), houdt geschrapte bronnen (Praxis, Gamma) buiten de deur, en controleert beide kanten van de string-koppeling: elke kolom heeft een spec die exact die key wegschrijft, én elke spec hoort bij een kolom. Dat laatste is geen theorie — `RunHordeurenAnalysisJob` globt `tests/*.spec.js`, dus een spec zonder kolom kost wél een queue-job en levert niets op. `globalTeardown.js` exporteert `COMPETITORS` en `SOURCES` naast de teardown-functie zodat de test ze niet hoeft te kopiëren.
